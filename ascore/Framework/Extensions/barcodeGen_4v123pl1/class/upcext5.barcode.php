 ...
 *--------------------------------------------------------------------
 * $Id: upcext5.barcode.php,v 1.1 2007/05/17 16:33:13 actsys Exp $
 * PHP5-Revision: 1.8
 *--------------------------------------------------------------------
...
class upcext5 extends BarCode {
	 ...
	 */
	function upcext5($maxHeight, &$color1, &$color2, $res, $text, $textfont) {
	...
	function draw(&$im) {
		...
		if ($error_stop === false) {
			...
			if ($error_stop === false) {
				...
				// Code
				for ($i = 0; $i < 5; $i++) {
					$this->DrawChar($im, upcext5::inverse($this->findCode($this->text[$i]), $this->codeParity[$this->checksumValue][$i]), 2);
					if ($i < 4) {
