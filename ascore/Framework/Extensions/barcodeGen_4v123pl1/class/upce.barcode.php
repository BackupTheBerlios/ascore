 ...
 *--------------------------------------------------------------------
 * $Id: upce.barcode.php,v 1.1 2007/05/17 16:33:10 actsys Exp $
 * PHP5-Revision: 1.10
 *--------------------------------------------------------------------
...
class upce extends BarCode {
	 ...
	 */
	function upce($maxHeight, &$color1, &$color2, $res, $text, $textfont) {
		...
		if ($error_stop === false) {
			...
			if ($error_stop === false) {
				...
				if ($error_stop === false) {
					...
					$c = strlen($upce);
					for ($i = 0; $i< $c; $i++) {
						$this->DrawChar($im, upce::inverse($this->findCode($upce[$i]), $this->codeParity[$this->text[0]][$this->checksumValue][$i]), 2);
					}
