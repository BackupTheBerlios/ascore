 ...
 *--------------------------------------------------------------------
 * $Id: upcext2.barcode.php,v 1.1 2007/05/17 16:33:05 actsys Exp $
 * PHP5-Revision: 1.9
 *--------------------------------------------------------------------
...
class upcext2 extends BarCode {
	 ...
	 */
	function upcext2($maxHeight, &$color1, &$color2, $res, $text, $textfont) {
	...
	function draw(&$im) {
		...
		if ($error_stop === false) {
			...
			if ($error_stop === false) {
				...
				// Code
				for ($i = 0; $i < 2; $i++) {
					$this->DrawChar($im, upcext2::inverse($this->findCode($this->text[$i]), $this->codeParity[intval($this->text) % 4][$i]), 2);
					if ($i === 0) {
