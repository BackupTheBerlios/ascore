 ...
 *--------------------------------------------------------------------
 * $Id: ean13.barcode.php,v 1.1 2007/05/17 16:33:09 actsys Exp $
 * PHP5-Revision: 1.9
 *--------------------------------------------------------------------
...
class ean13 extends BarCode {
	 ...
	 * @param bool $book
	 */
	function ean13($maxHeight, &$color1, &$color2, $res, $text, $textfont, $book = false) {
	...
	function draw(&$im) {
		...
		if ($error_stop === false) {
			...
			if ($error_stop === false) {
				...
				for ($i = 0; $i < 5; $i++) {
					$this->DrawChar($im, ean13::inverse($this->findCode($temp_text[$i + 2]), $this->codeParity[$temp_text[0]][$i]), 2);
				}
