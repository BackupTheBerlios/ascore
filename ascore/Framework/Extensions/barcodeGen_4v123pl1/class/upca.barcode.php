 ...
 *--------------------------------------------------------------------
 * $Id: upca.barcode.php,v 1.1 2007/05/17 16:33:11 actsys Exp $
 * PHP5-Revision: 1.10
 *--------------------------------------------------------------------
...
class upca extends BarCode {
	var $keys = array(), $code = array(), $codeParity = array();
	 ...
	 */
	function upca($maxHeight, &$color1, &$color2, $res, $text, $textfont) {
	...
	function draw(&$im) {
		...
		if ($error_stop === false) {
			...
			if ($error_stop === false) {
				...
				for ($i = 0; $i < 5; $i++) {
					$this->DrawChar($im, upca::inverse($this->findCode($temp_text[$i + 2]), $this->codeParity[$temp_text[0]][$i]), 2);
				}
	...
	function getStartPosition($position_start = 0) {
		if (is_a($this->textfont, 'Font')) {
